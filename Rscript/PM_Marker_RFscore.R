...
# Last update: 2017-2-22, Xiaoquan Su, Zheng Sun
...
for (i in 2:ncol(data_map)) {
  ...
  IL <- ggplot(framebm,aes(y=reorder(row.names(framebm),framebm$MeanDecreaseAccuracy),x=framebm$MeanDecreaseAccuracy))+geom_point(size=3,colour="#D55E00")+geom_segment(aes(yend=row.names(framebm)),xend=0,colour="#D55E00")+theme_bw()+theme(panel.grid.major.y=element_blank(),panel.grid.major.x=element_blank())+xlab(paste("Mean decrease in accuracy")) + ylab(paste("Features"))+ggtitle(paste("Random-Forest Score (error rate = ",round(error_rate, digits = 4)*100,"%)",sep="")) + theme(title=element_text(size=10))
  ...
  suppressMessages(ggsave(filename=outPDFfile,plot=IL,width=8 ,height = (dim(newframebm)[1]*0.15 + 1),limitsize =F))
  } 
}
