...
int main(int argc, char * argv[]){
    ...
    outscript << endl << "#Multivariate Statistical Analysis" << endl;           
           ...
           //Beta: taxa dist
           if ((Taxa_dist_type == 0) || (Taxa_dist_type == 2)){
           sprintf(command, "Rscript %s/PM_Bdiversity.R -m %s -d %s/taxa.dist -o %s -p taxa.dist -n %s", R_path.c_str(), Meta_file.c_str(), Dist_dir.c_str(), Beta_dir.c_str(), Taxa_dist_name.c_str());
           command_parallel_scripts.push_back(command);
           ...
           if ((Taxa_dist_type == 1) || (Taxa_dist_type == 2)){
           sprintf(command, "Rscript %s/PM_Bdiversity.R -m %s -d %s/taxa.unweighted.dist -o %s -p taxa.unweighted.dist -n %s", R_path.c_str(), Meta_file.c_str(), Dist_dir.c_str(), Beta_dir.c_str(), Taxa_dist_name_uw.c_str());
           command_parallel_scripts.push_back(command);
           command_parallel_titles.push_back("PM_Bdiversity.R");
           outscript << command << endl;                    
           }
           ...
           //Beta: func dist
           sprintf(command, "Rscript %s/PM_Bdiversity.R -m %s -d %s/func.dist -o %s -p func.dist -n %s", R_path.c_str(), Meta_file.c_str(), Dist_dir.c_str(), Beta_dir.c_str(), Func_dist_name.c_str());
           command_parallel_scripts.push_back(command);
           command_parallel_titles.push_back("PM_Bdiversity.R");
           outscript << command << endl;           
           }
